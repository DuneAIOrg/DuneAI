...
const generateReadme = async () => {
    ...
    const extractInfoFromSource = async (files) => {
        let info = '';
        files.forEach((file) => {
    ...
    const structure = await (0, exports.StructureDynamic)({ extractedInfo, introduction }).run({});
    let allSections = '';
    // @ts-ignore
    for (const rawCurrentSection of structure.Structure.Structure.split(',')) {
        const currentSection = rawCurrentSection.trim();
        const sectionContent = await (0, exports.SectionDynamic)({ extractedInfo, structure, currentSection, allSections, introduction }).run({});
        // @ts-ignore
        allSections += sectionContent.SectionDynamic[currentSection];
    }
    const edit = await (0, exports.EditDynamic)({ allSections: `${introduction}\n\n${allSections}` }).run({});
    // @ts-ignore
